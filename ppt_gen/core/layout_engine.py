...
class LayoutEngine:
    ...
    def get_group_rect_in_inches(self, slots: List[Slot]) -> Tuple[float, float, float, float]:
        """Compute the bounding box encompassing all slots belonging to the same visual group."""
        if not slots:
        ...
        # We can add a slight padding outer bounds if needed, but since it's aligned to cell boundaries,
        # it wraps the contents perfectly.
